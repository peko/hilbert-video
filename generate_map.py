...
def generate_zoom_out(map_dir):
    ...
    for z in range(start_z,1,-1):
        for x in range(0,64>>d,2):
            for y in range(0,64>>d,2):
                out = Image.new("RGB", (256,256))
                im00 = imop(f"{map_dir}/{z}-{x+0}-{y+0}.png", image_size//2)
                im10 = imop(f"{map_dir}/{z}-{x+1}-{y+0}.png", image_size//2)
                im01 = imop(f"{map_dir}/{z}-{x+0}-{y+1}.png", image_size//2)
                im11 = imop(f"{map_dir}/{z}-{x+1}-{y+1}.png", image_size//2)
                out.paste(im00, (  0,   0))
                out.paste(im10, (128,   0))
                out.paste(im01, (  0, 128))
                out.paste(im11, (128, 128))
                # out = out.filter(ImageFilter.SHARPEN)
                tile = f"{map_dir}/{z-1}-{x//2}-{y//2}.png"
