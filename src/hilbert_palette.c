...
Palette *
QuantizedPaletteForImage(rgb* image, int resolution, int size) {
    rgb* data = malloc(sizeof(MCTriplet) * resolution);
    rgb* colors;
    Palette* palette;
    ...
    colors = MCQuantizeData(data, resolution, log2(size));

    palette = malloc(sizeof(Palette));
    palette->size = size;
    palette->colors = malloc(sizeof(rgb) * size);
